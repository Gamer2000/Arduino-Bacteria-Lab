...
// =====================================================
// Bacteria Lab v2.0 RESIDUAL BIOMASS
...
// - атакующая бактерия отмечается квадратной рамкой;
...
// - размножение облегчено, чтобы популяция могла увеличиваться;
...
void redrawWorld()
{
  ...
  for (byte i = 0; i < MAX_BACTERIA; i++)
  {
    if (bacteria[i].alive)
    {
      ...
      if (bacteria[i].attackCooldown > 10)
      {
        Tft.drawRectangle(
          (int)bacteria[i].x - 6,
          (int)bacteria[i].y - 6,
