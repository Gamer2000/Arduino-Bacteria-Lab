...
// - статистика переключается между генами, родословной и численностью видов;
// - сохранены хемотаксис, мутации, родословная и сенсорные кнопки.
// =====================================================
...
unsigned long statisticsPageTimer = 0;
byte statisticsPage = 0;
...
void reproduce(byte parentIndex)
{
  ...
  int reproductionThreshold = 115;
  int reproductionCost = 55;
...
void checkDeath(byte index)
{
  ...
  // Клеточные остатки бактерии становятся пищей
  createFoodAt(
    deathX,
    deathY
  );
...
void drawStatistics()
{
  ...
  for (byte i = 0; i < MAX_BACTERIA; i++)
  {
    ...
    if (bacteria[i].species < 4)
    {
      speciesCount[bacteria[i].species]++;
    }

    if (bacteria[i].generation > maxGeneration)
    {
      maxGeneration = bacteria[i].generation;
    }

    if (bacteria[i].children > maxChildren)
    {
      maxChildren = bacteria[i].children;
    }

    if (bacteria[i].age > oldestAge)
    {
      oldestAge = bacteria[i].age;
      oldestId = bacteria[i].id;
    }
  }
  ...
  // Переключаем вторую строку каждые 3 секунды.
  if (millis() - statisticsPageTimer >= 3000)
  {
    statisticsPageTimer = millis();
    statisticsPage++;
  ...
  else
  {
    snprintf(
      ...
      sizeof(statusLine2),
      "E:%u S:%u V:%u B:%u",
      speciesCount[SPECIES_EXPLORER],
      speciesCount[SPECIES_SPRINTER],
      speciesCount[SPECIES_SURVIVOR],
      speciesCount[SPECIES_BREEDER]
    );
  }
  ...
  {
    Tft.drawString(statusLine2, 2, 18, 1, YELLOW);
  }
  else
  {
    Tft.drawString(statusLine2, 2, 18, 1, GREEN);
  }
...
void showBacteriaInfo(byte index)
{
  ...
  Tft.drawString(line, 25, 157, 2, WHITE);
  ...
  Tft.drawString(line, 25, 182, 2, WHITE);

  snprintf(line, sizeof(line), "Turn:%u", b.turnGene);
  Tft.drawString(line, 25, 207, 2, WHITE);
