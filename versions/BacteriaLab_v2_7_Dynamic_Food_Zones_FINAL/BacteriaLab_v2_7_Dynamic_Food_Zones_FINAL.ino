...
// =====================================================
// Bacteria Lab v2.7 DYNAMIC FOOD ZONES
...
#define FOOD_ZONE_INTERVAL 18000UL
#define FOOD_ZONE_RADIUS 55
#define FIELD_LEFT   8
...
unsigned long environmentTimer = 0;
byte foodZone = 0;
...
// =====================================================
// Центр текущей пищевой зоны
// =====================================================

void getFoodZoneCenter(int &centerX, int &centerY)
{
  if (foodZone == 0)
  {
    centerX = 55;
...
{
  if (millis() - environmentTimer < FOOD_ZONE_INTERVAL)
  {
    return;
  }

  environmentTimer = millis();
  foodZone++;
...
void createFood(byte index)
{
  ...
  getFoodZoneCenter(centerX, centerY);

  if (random(0, 100) < 80)
  {
    food[index].x =
      centerX + random(-FOOD_ZONE_RADIUS, FOOD_ZONE_RADIUS + 1);

    food[index].y =
      centerY + random(-FOOD_ZONE_RADIUS, FOOD_ZONE_RADIUS + 1);

    food[index].x =
      limitValue(food[index].x, FIELD_LEFT + 10, FIELD_RIGHT - 10);

    food[index].y =
      limitValue(food[index].y, FIELD_TOP + 10, FIELD_BOTTOM - 10);
  }
  else
  {
    food[index].x =
...
// =====================================================
// Накопление и ремонт повреждений
// =====================================================

void updateDamage(byte index)
{
  Bacteria &b = bacteria[index];

  int damageChance = 900;
  ...
  // Быстрые бактерии сильнее изнашиваются.
  if (b.speed == 3)
  {
    damageChance -= 40;
  }
  ...
  // Обычное микроповреждение.
  if (random(0, damageChance) == 0)
  {
    if (b.damage < 250)
    {
      b.damage++;
  ...
  // Редкое сильное внешнее событие.
  if (random(0, 12000) == 0)
  {
    b.damage =
      limitValue(
  ...
  // Ремонт требует энергии.
  if (
    b.damage > 0 &&
    b.energy > 55 &&
    random(0, 24) < repairPower
  )
  {
    b.damage--;
    b.energy--;
  ...
  // Сильные повреждения увеличивают расход энергии.
  if (
    b.damage >= 8 &&
    (frameCounter % 8) == 0
  )
...
void showBacteriaInfo(byte index)
{
  ...
  else
  {
    ...
    Tft.drawString(line, 22, 180, 2, RED);
    ...
    Tft.drawString(line, 22, 210, 2, WHITE);

    snprintf(line, sizeof(line), "Sp:%u Se:%u Tu:%u", b.speed, b.sensor, b.turnGene);
    Tft.drawString(line, 22, 240, 2, WHITE);
