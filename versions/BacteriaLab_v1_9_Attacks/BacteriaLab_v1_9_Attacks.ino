...
// - бактерии могут атаковать соседей при нехватке энергии;
// - атака отнимает энергию у жертвы и передаёт часть нападающему;
// - добавлены счётчики атак и убийств;
// - на главном экране оставлена краткая статистика A/B/D/F;
...
void checkAttack(byte attackerIndex)
{
  ...
  // Короткая белая вспышка в точке атаки.
  Tft.drawCircle(
    (int)victim.x,
    (int)victim.y,
    6,
...
void showBacteriaInfo(byte index)
{
  ...
  else
  {
    ...
    snprintf(line, sizeof(line), "Kills:%u", b.kills);
